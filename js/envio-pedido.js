...
(function (root, factory) {
  ...
  // datos: { pedido, proveedor, lineas:[{codigo,insumo,cantidad,unidad}], unidades:{codigo:unidad}, empresa, intento, detalleWhatsapp, enlacePdf }
  //   enlacePdf: enlace CORTO (https://<dominio>/p/<codigo>) para abrir el PDF de la orden desde WhatsApp; sin emojis (en WhatsApp Web llegaban como "�").
  ...
  function armarMensajes(datos) {
    ...
    const reenvio = Number(datos && datos.intento) > 1, detalleWa = !(datos && datos.detalleWhatsapp === false), enlacePdf = t(datos && datos.enlacePdf);
    ...
    const textoWhatsapp = [
      ...
      ...(obs ? [`Observaciones: ${obs}`, ''] : []),
      ...(enlacePdf ? ['Abrir orden de compra (PDF):', enlacePdf, ''] : []),
  ...
  // Codigo del enlace corto: 10 caracteres sin ambiguos (sin 0 O 1 l I), ~49 bits: no se puede adivinar. rng: () => [0,1)
  function codigoPdf(rng) { const A = 'abcdefghjkmnpqrstuvwxyz23456789'; let s = ''; for (let i = 0; i < 10; i++) s += A[Math.floor(rng() * A.length) % A.length]; return s; }
  const enlaceCorto = (origen, codigo) => String(origen || '').replace(/\/+$/, '') + '/p/' + codigo;
  ...
  return { validarCorreo, validarWhatsapp, contacto, fechaCorta, cantidadTexto, unidadDe, armarMensajes, enlaceWhatsapp, rutaPdf, codigoPdf, enlaceCorto,
